...
export default function ProfilePage() {
  ...
  const canChangeUsername = () => {
    if (isAdmin) return true // Admin bisa ganti kapan saja
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', paddingBottom: 40 }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Kelola informasi pribadi dan pengaturan akun Anda</p>
        </div>
      </div>
      ...
      <div className="grid-2">
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 20, marginBottom: 4 }}>{profile.display_name || profile.username || 'Tanpa Nama'}</h2>
          ...
          <div style={{ width: '100%', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 13 }}>Email Terverifikasi</span>
            </div>
          </div>
        </div>
        ...
        {/* Edit Form */}
        <div className="card">
          <h3 className="section-title">Edit Profil</h3>
          <form onSubmit={handleUpdate}>
            
            <div className="form-group">
              <label className="form-label">Nama Akun (Display Name)</label>
              <input 
                type="text" 
                className="form-input" 
                placeholder="Nama Anda yang tampil publik"
                value={profile.display_name}
                onChange={e => setProfile({ ...profile, display_name: e.target.value })}
              />
              <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Opsional. Bebas diubah kapan pun.</p>
            </div>
            ...
            <div className="form-group">
              <label className="form-label" style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                {isAdmin ? (
                  ...
                  <span style={{ color: 'var(--red)', fontSize: 10 }}>Terkunci {daysUntilNextChange()} hari lagi</span>
                ) : null}
              </label>
      ...
      <div className="card" style={{ marginTop: 24, border: '1px solid var(--red)', background: 'rgba(239, 68, 68, 0.05)' }}>
        <h3 className="section-title" style={{ color: 'var(--red)' }}>Zona Berbahaya</h3>
        <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 16 }}>
          Menghapus akun akan menghilangkan semua data transaksi Anda secara permanen.
        </p>
        <button className="btn btn-danger">Hapus Akun & Data</button>
      </div>
    </div>
  )
}
